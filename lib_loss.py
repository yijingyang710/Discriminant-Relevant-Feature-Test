...
def cal_entropy_from_y(y_array, y_distribution, num_cls=10):
...
def cal_weighted_H(X, y, bound, num_cls=10): # weighted entropy
    classwise = class_distribution(y)
    ...
    if np.sum(X<bound)==0 or np.sum(X>=bound)==0:
        ...
        entropy = np.array([cal_entropy_from_y(left_y, classwise), cal_entropy_from_y(right_y, classwise)]).reshape(-1,1)
